...
- **Each instance controls one charger on one circuit.** Add a separate instance for each power meter / circuit you want to balance. The integration prevents the same power meter from being added twice — if you try, the setup wizard will show an "already configured" error. Multi-charger support within a single instance (with per-charger prioritization) is planned for [Phase 2](milestones/02-2026-05-22-multi-charger-plan.md).
...
in_post_step_window = (last_step_increase_at is not None
                       and (now - last_step_increase_at) <= max(ramp_up_time_s, 60 s))
