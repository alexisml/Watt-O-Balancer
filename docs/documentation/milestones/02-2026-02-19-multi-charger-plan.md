...
- **Maximum 3 chargers per power meter**: keeps the UI manageable while covering the majority of real-world use cases. The limit is defined by `MAX_CHARGERS = 3` in `const.py` and enforced in the options flow.
  ...
  > **Design note — dynamic N vs. fixed count**: HA config flows *do* support a looping single step (return the same `step_id` from within the handler; the frontend re-renders the form). A single re-entrant `charger` step with `description_placeholders` to inject the charger number ("Configuring charger 1 of N") would allow the user to add any number of chargers without hardcoding additional step handlers or `strings.json` entries. The trade-off is that the current fixed-step approach (`charger_1`, `charger_2`, `charger_3`) gives each step a distinct, statically-translated title, which is slightly cleaner in the HA UI. If the use case requires more than 3 chargers, the looping single-step pattern is the right approach — only `MAX_CHARGERS` in `const.py` and one handler in `config_flow.py` would need updating. No HA framework change is required.
