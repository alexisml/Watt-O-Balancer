...
| **Min EV current changed** | Same as above. If the new minimum is higher than the current target, charging stops immediately even while the meter is unavailable. If the new minimum exceeds the current commanded set-point during a ramp-up hold, the hold floor is advanced immediately to `min_ev_current`. | Instant. |
